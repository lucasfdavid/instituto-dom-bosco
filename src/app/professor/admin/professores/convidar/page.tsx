...
export default function AdminConvidarProfessor() {
  ...
  async function handleConvidar(e: React.FormEvent) {
    ...
    if (!response.ok || data.error) {
      setErro(data.error ?? `Erro ${response.status}`)
      setLoading(false)
      return
    }
  ...
  return (
    <div className="p-6 max-w-xl">
      <Link href="/professor/admin/professores" className="text-sm text-gray-400 hover:text-navy flex items-center gap-1 mb-6">
