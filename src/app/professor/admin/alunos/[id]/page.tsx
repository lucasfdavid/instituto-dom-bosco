...
export default function AdminAlunoDetalhe({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!profile) { router.push('/professor/admin/alunos'); return }
  ...
  async function handleExcluir() {
    ...
    if (error) {
      ...
      router.push('/professor/admin/alunos')
    }
  }
  ...
  return (
    <div className="p-6 max-w-xl">
      <div className="flex items-center justify-between mb-6">
        <Link href="/professor/admin/alunos" className="text-sm text-gray-400 hover:text-navy flex items-center gap-1">
          ← Voltar para alunos
        </Link>
        <button
