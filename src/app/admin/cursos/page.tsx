...
export default function AdminCursos() {
  ...
  async function handleAdicionar(e: React.FormEvent) {
    ...
    const { error } = await supabase
      .from('cursos')
      .insert({ nome: nome.trim(), categoria, ativo: true })

    if (error) {
      setErro('Erro ao adicionar curso.')
    } else {
      setNome('')
      await loadCursos()
    }
        ...
        {cursosPorCategoria.map(({ value, label, lista }) => (
          <div key={value} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-gray-50">
              ...
              <span className="ml-auto font-condensed text-xs text-gray-400">
                ...
                {lista.map(curso => (
                  <div key={curso.id} className="flex items-center justify-between px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-sm font-medium ${curso.ativo ? 'text-navy' : 'text-gray-400 line-through'}`}>
                        {curso.nome}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`font-condensed text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${
                          curso.ativo
